...
const genPaginationSmallStyle: GenerateStyle<PaginationToken, CSSObject> = (
  token,
) => {
  const { componentCls } = token;
...
const genPaginationLargeStyle: GenerateStyle<PaginationToken, CSSObject> = (
  token,
) => {
  const { componentCls } = token;
...
const genPaginationSimpleStyle: GenerateStyle<PaginationToken, CSSObject> = (
  ...
  const [, varRef] = genCssVar(asCls, 'pagination');
...
const genPaginationInputVariantStyle: GenerateStyle<
  PaginationToken,
  CSSObject
> = (token) => {
  const { componentCls } = token;
...
const genPaginationStyle: GenerateStyle<PaginationToken, CSSObject> = (
  ...
  const [varName, varRef] = genCssVar(asCls, 'pagination');
...
const genPaginationFocusStyle: GenerateStyle<PaginationToken, CSSObject> = (
  token,
) => {
  const { componentCls } = token;
