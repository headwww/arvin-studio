...
import { Keyframes, unit } from '@arvin-studio/cssinjs';
...
const skeletonClsLoading = new Keyframes(`as-skeleton-loading`, {
  '0%': {
...
function genSkeletonElementInputSize(
  size: number,
  calc: CSSUtil['calc'],
): CSSObject {
  return {
...
const genSkeletonElementAvatar: GenerateStyle<SkeletonToken, CSSObject> = (
  token,
) => {
  const {
    skeletonAvatarCls,
...
const genSkeletonElementInput: GenerateStyle<SkeletonToken, CSSObject> = (
  token,
) => {
  const {
    controlHeight,
...
const genSkeletonElementShape: GenerateStyle<SkeletonToken, CSSObject> = (
  token,
) => {
...
const genSkeletonElementNode: GenerateStyle<SkeletonToken, CSSObject> = (
  token,
) => {
  return {
...
const genSkeletonElementImage: GenerateStyle<SkeletonToken, CSSObject> = (
  token,
) => {
...
function genSkeletonElementButtonShape(
  token: SkeletonToken,
  size: number,
  buttonCls: string,
): CSSObject {
...
function genSkeletonElementButtonSize(
  size: number,
  calc: CSSUtil['calc'],
): CSSObject {
  return {
...
const genSkeletonElementButton: GenerateStyle<SkeletonToken, CSSObject> = (
  token,
) => {
  const {
    borderRadiusSM,
