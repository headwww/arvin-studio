...
function genSelectInputVariableStyle(
  token: SelectToken,
  colors: VariableColors,
): CSSObject {
  const { componentCls, asCls } = token;

  const [varName] = genCssVar(asCls, 'select');

  const { border, borderHover, borderActive, borderOutline } = colors;

  const baseBG =
    colors.background || token.selectorBg || token.colorBgContainer;
...
function genSelectInputVariantStyle(
  token: SelectToken,
  variant: string,
  colors: VariableColors,
  ...
  return {
    [`&${componentCls}-${variant}`]: [
      genSelectInputVariableStyle(token, colors),
      {
        ...
        [`&${componentCls}-status-warning`]: genSelectInputVariableStyle(
          token,
          {
            ...colors,
...
function genSelectInputFocusVisibleStyle(
  token: SelectToken,
  outlineColor: string,
): CSSObject {
  return {
...
const genSelectInputStyle: GenerateStyle<SelectToken, CSSObject> = (token) => {
  const { componentCls, fontHeight, controlHeight, iconCls, asCls, calc } =
    token;
  const [varName, varRef] = genCssVar(asCls, 'select');
    ...
    [componentCls]: [
      {
        ...
        [varName('padding-horizontal')]: calc(token.paddingSM)
          .sub(token.lineWidth)
          .equal(),
        ...
        '&-sm': {
          ...
          [varName('padding-horizontal')]: calc(token.paddingXS)
            .sub(token.lineWidth)
            .equal(),
