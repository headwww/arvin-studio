...
import { canUseDom, useMutateObserver } from '@arvin-studio/headless';
import { omit } from '@arvin-studio/kit';

import { useToken } from '../theme/internal';
import { WatermarkContextProvider } from './context';
...
export interface WatermarkFont {
  ...
  fontStyle?: 'italic' | 'none' | 'normal' | 'oblique';
  fontWeight?: 'bold' | 'bolder' | 'lighter' | 'normal' | number;
...
const Watermark = defineComponent<WatermarkProps>(
  (props = defaults, { slots, attrs }) => {
    ...
    const mergedZIndex = computed(
      () =>
        props.zIndex ?? token.value.zIndexPopupBase - WATERMARK_Z_INDEX_OFFSET,
    );
    const onRemove = computed(() => props.onRemove);
    const color = computed(() => props.font?.color ?? token.value.colorFill);
    const fontSize = computed(
      () => props?.font?.fontSize ?? token.value?.fontSizeLG,
    );
    ...
    const contentLines = computed(() =>
      getContentLines(props.content, mergedFont.value as any),
    );
    ...
    const getMarkSize = (ctx: CanvasRenderingContext2D) => {
      ...
      if (!props.image && ctx.measureText && contentLines.value.length > 0) {
    ...
    const renderWatermark = () => {
      ...
      if (ctx) {
        ...
        const drawCanvas = (
          drawContent?: HTMLImageElement | typeof contentLines.value,
        ) => {
    ...
    const [appendWatermark, removeWatermark, isWatermarkEle] = useWatermark(
      markStyle,
      onRemove,
    );
    ...
    const onMutate = (mutations: MutationRecord[]) => {
      mutations.forEach((mutation) => {
        if (reRendering(mutation, isWatermarkEle)) {
          ...
          mutation.target === container.value &&
    ...
    return () => {
      ...
      // ============================= Render =============================
      const childNode = props.inherit ? (
        <WatermarkContextProvider {...watermarkContext}>
          {children}
      ...
      return (
        <div
          class={[props.rootClass, (attrs as any).class]}
          ref={container}
