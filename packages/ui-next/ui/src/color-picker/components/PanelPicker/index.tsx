...
import { ColorPicker as VcColorPicker } from '@arvin-studio/headless';
...
export default defineComponent(
  () => {
    ...
    const lockedColor = shallowRef<AggregationColor>(
      panelPickerContext.value?.value ?? new AggregationColor(''),
    );
    ...
    watch(
      ...
      () => {
        if (!isSingle()) {
          lockedColor.value = colors()[
            panelPickerContext.value?.activeIndex || 0
          ]?.color as any;
        }
      },
      { immediate: true },
    ...
    const mergedPickerColor = () =>
      pickerColor.value?.equals(activeColor())
        ? activeColor()
        : pickerColor.value;
    ...
    const onPickerChange = (
      ...
      pickerColor.value = nextColor.isGradient()
        ? (nextColor.getColors() as any)[activeIndex].color
        : nextColor;
      panelPickerContext.value?.onChange?.(nextColor, true);
    };

    const onInternalChangeComplete = (
      nextColor: AggregationColor,
      info?: { type?: 'alpha' | 'hue'; value?: number },
    ) => {
      ...
      return (
        <>
          ...
          <VcColorPicker
            ...
            onChange={(colorValue: any, info: any) => {
              onPickerChange(colorValue as any, info as any);
            }}
            onChangeComplete={(colorValue: any, info: any) => {
