...
const genTransferCustomizeStyle: GenerateStyle<TransferToken, CSSObject> = (
  token,
) => {
  const { asCls, componentCls, listHeight, controlHeightLG } = token;

  const tableCls = `${asCls}-table`;
  const inputCls = `${asCls}-input`;
...
function genTransferStatusColor(
  token: TransferToken,
  color: string,
): CSSObject {
...
const genTransferStatusStyle: GenerateStyle<TransferToken, CSSObject> = (
  token,
) => {
...
const genTransferListStyle: GenerateStyle<TransferToken, CSSObject> = (
  token,
) => {
  const {
    componentCls,
...
const genTransferRTLStyle: GenerateStyle<TransferToken, CSSObject> = (
  token,
) => {
  const { componentCls } = token;
  return {
