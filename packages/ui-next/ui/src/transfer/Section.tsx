...
const TransferSection = defineComponent<TransferListProps<KeyWiseTransferItem>>(
  (props) => {
    ...
    const renderItem = (item: KeyWiseTransferItem) => {
      ...
      const renderedEl =
        mergedLabel ??
        (isRenderResultPlain ? renderResult.label : renderResult);
    ...
    const filteredItems = computed(() => {
      ...
      dataSource.value.forEach((item) => {
        ...
        if (
          filterValue.value &&
          !matchFilter(renderedItem.renderedText, item)
        ) {
          return;
        }
    ...
    const checkedActiveItems = computed(() =>
      filteredItems.value.filterItems.filter(
        (item) => props.checkedKeys.includes(item.key) && !item.disabled,
      ),
    );
    ...
    const checkStatus = computed(() => {
      ...
      if (
        filteredItems.value.filterItems.every(
    ...
    const renderListBody = () => {
      ...
      const raw = props.renderList
        ? props.renderList({
            ...listProps,
            onItemSelect: (key: any, check: any) =>
              listProps.onItemSelect(key, check),
          })
        : null;

      const slotNodes =
        // oxlint-disable-next-line eqeqeq
        raw == null ? [] : filterEmpty(Array.isArray(raw) ? raw : [raw]);
      ...
      const bodyNode = customize ? (
        <div class={`${listPrefixCls.value}-body-customize-wrapper`}>
          {bodyContent}
        </div>
      ) : filteredItems.value.filterItems.length > 0 ? (
        bodyContent
      ) : (
    ...
    return () => {
      const checkBox = (
        <Checkbox
          ...
          disabled={dataSource.value.every((d) => d.disabled) || props.disabled}
      ...
      const checkAllCheckbox =
        !props.showRemove && !props.pagination && checkBox;

      let items: MenuProps['items'];

      items = props.showRemove
        ? [
            props.pagination
              ? {
                  key: 'removeCurrent',
        ...
        : [
            {
              ...
              label:
                checkStatus.value === 'all'
                  ...
                  : props.selectAll,
              onClick() {
                const keys = getEnabledItemKeys(
                  filteredItems.value.filterItems,
                );
                props.onItemSelectAll?.(
                  keys,
                  keys.length !== props.checkedKeys.length,
                );
              },
            },
            props.pagination
              ? {
                  ...
                  onClick() {
                    ...
                    props.onItemSelectAll?.(
                      getEnabledItemKeys(
                        pageItems.map((entity) => entity.item),
                      ),
                      true,
                    );
                  },
      ...
      const dropdown = (
        <Dropdown
          ...
          menu={{ items }}
        >
          {isValidIcon(props.selectionsIcon) ? (
      ...
      return (
        ...
        >
          ...
          >
            ...
            <span
              class={clsx(
                `${listPrefixCls.value}-header-title`,
                props.classes?.title,
              )}
