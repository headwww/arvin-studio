...
function camelize(str: string) {
  return str.replaceAll(/-(\w)/g, (_, c: string) => (c ? c.toUpperCase() : ''));
}
...
function normalizeSlotValue(value: any) {
  const nodes = filterEmpty(value).filter(
    (node) =>
      node !== undefined &&
      node !== null &&
      (typeof node !== 'object' || (node as any).type !== Comment),
  );
...
export function resolveVNodeProps(
  vnode: VNode,
  name?: string,
  markerKey?: string,
) {
  ...
  if (normalizedPropsOptions.length > 0) {
    const [options, needCastKeys] = normalizedPropsOptions as [
      Record<string, NormalizedProp>,
      string[],
    ];
...
export function resolveSlotsNode<T = any>(
  ...
  const nodes = filterEmpty(slots?.[slotName]?.() ?? []).filter(
    (node: any) => node !== undefined && node !== null && node.type !== Comment,
  );
  if (nodes.length === 0) {
    return [];
  }
