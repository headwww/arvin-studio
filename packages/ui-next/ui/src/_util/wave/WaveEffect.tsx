...
export const WaveEffect = defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const waveVarName = computed(
      () => genCssVar(rootPrefixCls.value, 'wave')[0],
    );
    const divRef = shallowRef<HTMLDivElement>();
    const waveColor = ref<null | string>(null);
    ...
    const classes = computed(() => {
      return clsx(props.className, {
        'wave-quick': isSmallComponent.value,
      });
    });
    ...
    function triggerEffect() {
      ...
      element.classList.add('wave-motion-appear', 'wave-motion-appear-active');
    }
    ...
    return () => {
      ...
      return <div class={classes.value} ref={divRef} style={waveStyle.value} />;
    };
  },
});
