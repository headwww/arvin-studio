...
const genBaseStyle: GenerateStyle<SliderToken, CSSObject> = (token) => {
  ...
  return {
    [componentCls]: {
      ...
      [`&-tooltip ${asCls}-tooltip-container`]: {
        minWidth: 'unset',
      },
    },
  };
...
function genDirectionStyle(token: SliderToken, horizontal: boolean): CSSObject {
  ...
  const railPadding: keyof CSSProperties = horizontal
    ? 'paddingBlock'
    : 'paddingInline';
  const full: keyof CSSProperties = horizontal ? 'width' : 'height';
  const part: keyof CSSProperties = horizontal ? 'height' : 'width';
  const handlePos: keyof CSSProperties = horizontal
    ? 'insetBlockStart'
    : 'insetInlineStart';
  const markInset: keyof CSSProperties = horizontal
    ? 'top'
    : 'insetInlineStart';
...
export const prepareComponentToken: GetDefaultToken<'Slider'> = (token) => {
  ...
  const handleActiveOutlineColor = new FastColor(handleActiveColor)
    .setA(0.2)
    .toRgbString();
