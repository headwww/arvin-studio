...
const prepareMessageToken: (
  token: Parameters<GenStyleFn<'Message'>>[0],
  ...
  return mergeToken<NotificationToken>(
    ...
    ),
    {
      ...
      notificationPadding:
        token.contentPadding as NotificationToken['notificationPadding'],
...
const generateMessageStackStyle: GenerateStyle<NotificationToken> = (token) => {
  ...
  const { '&::after': _hoverAfterStyle, ...messageNoticeStyle } =
...
const generateMessagePurePanelStyle: GenerateStyle<NotificationToken> = (
  token,
) => {
  const { componentCls } = token;
  const noticeCls = `${componentCls}-notice`;
...
export default genStyleHooks(
  ...
  (token) => {
    ...
    return [
      sharedGenerateStyle(messageToken, {
        stackVisibleCount: 1,
