...
import type { Tab as VcTab } from '@arvin-studio/headless';

import type { Tab, TabPaneProps, TabsProps } from '..';

import { computed, isVNode } from 'vue';

import { filterEmpty, flattenChildren } from '@arvin-studio/headless';
...
function convertItem(item: Tab, index: number, slots?: any): VcTab {
  ...
  const label =
    labelRender.length > 0 ? labelRender : getSlotPropsFnRun(item, {}, 'label');
...
export default function useLegacyItems(
  ...
  return computed<VcTab[]>(() => {
    ...
    if (itemsValue && itemsValue.length > 0) {
      return itemsValue.map((item, index) =>
        convertItem(item, index, slots),
      ) as VcTab[];
    }
    ...
    const childrenItems = children.map((node: any, index: number) => {
      ...
      const props = (node.props || {}) as TabPaneProps & {
