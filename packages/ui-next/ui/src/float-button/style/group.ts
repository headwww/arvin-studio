...
const genGroupStyle: GenerateStyle<FloatButtonToken, CSSObject> = (token) => {
  const { componentCls, asCls, floatButtonSize, padding } = token;

  const groupCls = `${componentCls}-group`;
  const listCls = `${groupCls}-list`;

  const [varName, varRef] = genCssVar(asCls, 'float-btn');
    ...
    [groupCls]: [
      ...
      // ==============================================================
      {
        [varName('list-transform-start')]:
          `translate(0,${unit(floatButtonSize)})`,
        [varName('list-trigger-offset')]:
          `calc(${unit(floatButtonSize)} + ${unit(padding)})`,
      },
      ...
      // ==============================================================
      {
        ...
        '&-left': {
          [listCls]: {
            [varName('list-transform-start')]:
              `translate(${unit(floatButtonSize)}, 0)`,
