...
import { VerticalAlignTopOutlined } from '@arvin-studio/icons';
...
export interface BackTopProps /* @vue-ignore */
  extends
    BackTopEmitsProps,
    ComponentBaseProps,
    Omit<FloatButtonProps, 'target'> {
  duration?: number;
...
>(
  (props, { attrs, slots, emit, expose }) => {
    const { backTopIcon } = useComponentBaseConfig(
      'floatButton',
      props,
      ['backTopIcon'],
    ...
    const { target, visibilityHeight, duration } = toPropsRefs(
      props,
      'target',
      'visibilityHeight',
      'duration',
    ...
    expose({
      nativeElement: computed(
        () => floatButtonRef.value?.nativeElement ?? null,
      ),
    });

    const getDefaultTarget = () =>
      floatButtonRef.value?.nativeElement?.ownerDocument ??
      (typeof window === 'undefined' ? undefined : window);
    const container = shallowRef<Document | HTMLElement | null | Window>(null);
    ...
    const mergedShape = computed<FloatButtonShape>(
      () => groupContext?.value?.shape ?? props.shape ?? 'circle',
    );
