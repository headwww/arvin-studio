...
type MentionsToken = {
   ...
   */
  itemPaddingVertical: number | string;
} & FullToken<'Mentions'> &
  SharedInputToken;
...
const genMentionsStyle: GenerateStyle<MentionsToken, CSSObject> = (token) => {
  const { componentCls, colorText, asCls, colorTextDisabled, calc } = token;
  const [varName, varRef] = genCssVar(asCls, 'cmp-mentions');
  return {
    [componentCls]: [
      // =========================== Common ===========================
      resetComponent(token),
