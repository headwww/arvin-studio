...
function getStackNoticeClipPath(offset: number | string) {
  return `inset(${offset} ${offset} ${offset} ${offset})`;
}

/** Generate shared list content and motion base styles. */
const genNotificationListContentStyle: GenerateStyle<
  NotificationToken,
  CSSObject
> = (token) => {
  const {
...
function genNotificationListStyle<Token extends NotificationToken>(
  ...
  const listWidth = config.listWidthKey
    ? token
        ...
        .equal()
    : '100%';
  const stackVisibleCount =
    config.stackVisibleCount ?? DEFAULT_COLLAPSED_STACK_VISIBLE_COUNT;
  const noticeBeyondStackVisibleCountCls = `${noticeCls}:nth-last-child(n + ${
    stackVisibleCount + 1
  })`;
  const stackShadowClipOffset = unit(
