...
const genStepsProgressStyle: GenerateStyle<StepsToken, CSSObject> = (token) => {
  const { calc, asCls, componentCls, lineWidthBold, motionDurationSlow } =
    token;
  ...
  return {
    [`${componentCls}${componentCls}-with-progress`]: {
      ...
      [`${itemCls}-progress-icon`]: {
        '&-svg': {
          [varName('svg-size')]: calc(enhanceSize)
            .mul(2)
            .add(varRef('icon-size'))
            .equal(),
          [varName('icon-size-ptg-unitless')]:
            `calc(100 / tan(atan2(${varRef('svg-size')}, 1px)))`,
        ...
        '&-circle': {
          lineHeight: varRef('icon-size-ptg-unitless'),
          strokeWidth: calc(varRef('icon-size-ptg-unitless'))
            .mul(lineWidthBold)
            .equal(),
