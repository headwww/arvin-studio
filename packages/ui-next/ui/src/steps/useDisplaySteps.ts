...
function getEllipsisStep(
  ...
  const prevKey = (items[prevIndex]!.key ?? prevIndex) as number | string;
  const nextKey = (items[nextIndex]!.key ?? nextIndex) as number | string;
  const hasError = items
    .slice(prevIndex + 1, nextIndex)
...
function getCollapsedIndexes(
  ...
  for (
    let distance = 1;
    indexes.size < targetCount && distance < total;
    distance += 1
  ) {
    const candidates = [
  ...
  return Array.from(indexes)
    ...
    .flatMap((index, order, sortedIndexes) =>
      order > 0 && index - sortedIndexes[order - 1]! > 1
        ? [null, index]
        : [index],
    );
}
...
export default function useDisplaySteps(
  ...
  const canApplyMaxCount = computed(
    () =>
      maxCount.value !== undefined &&
      maxCount.value >= 3 &&
      mergedItems.value.length > maxCount.value,
  );
  ...
  const displaySteps = computed<DisplayStep[]>(() => {
    if (!canApplyMaxCount.value) {
      return mergedItems.value.map((item, originIndex) => ({
        item,
        originIndex,
      }));
    }

    const collapsedIndexes = getCollapsedIndexes(
  ...
  const mappedDisplayCurrent = computed(() => {
    const displayCurrent = displaySteps.value.findIndex(
      (step) => step.originIndex === mappedCurrent.value,
    );
    return displayCurrent === -1 ? mappedCurrent.value : displayCurrent;
  });

  const displayItems = computed(() =>
    displaySteps.value.map((step) => step.item),
  );
