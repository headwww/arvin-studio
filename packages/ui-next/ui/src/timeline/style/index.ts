    ...
    [componentCls]: [
      ...
      // ==============================================================
      {
        ...
        [itemCls]: {
          ...
          [varName('item-icon-dot-bg-color-origin')]: varRef(
            'item-icon-dot-bg-color',
          ),
          [varName('item-icon-dot-bg-color-custom')]: dotBg,

          [varName('icon-size')]: varRef(
            'icon-dot-size-custom',
            varRef('icon-dot-size-origin'),
          ),
...
const genVerticalStyle: GenerateStyle<TimelineToken, CSSObject> = (token) => {
  const { calc, componentCls, itemPaddingBottom, margin, asCls } = token;
  const itemCls = `${componentCls}-item`;

  const [, stepsVarRef] = genCssVar(asCls, 'cmp-steps');
  const [timelineVarName, timelineVarRef] = genCssVar(asCls, 'timeline');

  return {
    [`${componentCls}:not(${componentCls}-horizontal)`]: {
      [timelineVarName('head-span')]: '12',
      [timelineVarName('head-span-ptg')]:
        `calc(${timelineVarRef('head-span')} / 24 * 100%)`,
