type BaseNamePath = (boolean | number | string)[] | boolean | number | string;
...
> = ParentNamePath['length'] extends 5
  ...
  : // Follow code is batch check if `Store` is base type
    ...
    ? ParentNamePath['length'] extends 0
      ? BaseNamePath | Store // Return `BaseNamePath` instead of array if `ParentNamePath` is empty
      : Store extends any[]
      ...
      : keyof Store extends never // unknown
        ...
        : {
            ...
            [FieldKey in keyof Store]: Store[FieldKey] extends Function
              ? never
              :
                  | [...ParentNamePath, FieldKey] // Exist `ParentNamePath`, connect it // If `ParentNamePath` is empty, it can use `FieldKey` without array path
                  | DeepNamePath<
