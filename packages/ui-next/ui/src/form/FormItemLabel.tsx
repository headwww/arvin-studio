...
import { QuestionCircleOutlined } from '@arvin-studio/icons';
...
const FormItemLabel = defineComponent<
  FormItemLabelProps & { prefixCls: string; required?: boolean }
>(
  (props, { slots }) => {
    ...
    return () => {
      ...
      const mergedLabelCol: ColPropsWithClass =
        labelCol || contextLabelCol || ({} as ColPropsWithClass);
      const mergedLabelAlign: FormLabelAlign = (labelAlign ||
        contextLabelAlign) as FormLabelAlign;
      ...
      if (tooltipProps) {
        ...
        const icon = getSlotPropsFnRun({}, tooltipProps, 'icon') ?? (
