...
export interface AvatarProps /* @vue-ignore */
  extends AvatarEmitsProps, ComponentBaseProps {
  alt?: string;
...
>(
  (props = defaults, { slots, attrs }) => {
    ...
    const needResponsive = computed(() => {
      return Object.keys(
        typeof size.value === 'object' ? size.value || {} : {},
      ).some((key) => responsiveArray.includes(key as Breakpoint));
    });
    ...
    return () => {
      ...
      const responsiveSizeStyleFn = () => {
        ...
        const currentSize =
          (size.value as any)[currentBreakpoint] || size.value;
        return currentSize
          ? {
              width: `${currentSize}px`,
              height: `${currentSize}px`,
              fontSize:
                currentSize && (icon || children.length > 0)
                  ? `${currentSize / 2}px`
                  : '18px',
            }
          : {};
      ...
      const classString = clsx(
        ...
        `${prefixCls.value}-${mergedShape}`,
        {
          [`${prefixCls.value}-image`]:
            hasImageElement || (src && isImgExist.value),
