  ...
  (props, { slots, attrs }) => {
    ...
    return () => {
      ...
      if (mergeCount && mergeCount < numOfChildren) {
        ...
        const childrenHidden = childrenWithProps.slice(
          mergeCount,
          numOfChildren,
        );
        ...
        const popoverProps: PopoverProps = {
          ...
          rootClass: clsx(
            `${groupPrefixCls.value}-popover`,
            max?.popover?.rootClass,
          ),
        };

        childrenShow.push(
          <Popover destroyOnHidden key="avatar-popover-key" {...popoverProps}>
            <Avatar
              style={mergeStyle}
            >{`+${numOfChildren - mergeCount}`}</Avatar>
          </Popover>,
        );
