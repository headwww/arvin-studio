...
export default function useMergedPickerSemantic<
  P extends AnyObject = AnyObject,
>(
  pickerType: 'datePicker' | 'rangePicker' | 'timePicker',
  ...
  const contextClassNames = computed(
    () => contextConfig.value?.classes as P['classes'],
  );
  const contextStyles = computed(
    () => contextConfig.value?.styles as P['styles'],
  );
