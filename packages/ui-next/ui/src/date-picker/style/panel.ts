...
const genPickerCellInnerStyle: GenerateStyle<SharedPickerToken, CSSObject> = (
  token,
) => {
  const {
    pickerCellCls,
...
export const genPanelStyle: GenerateStyle<SharedPickerToken, CSSObject> = (
  token,
) => {
  const {
    componentCls,
    ...
    [componentCls]: {
      ...
      '&-time-panel': {
        ...
        '&-column': {
          ...
          '&-active': {
            background: new FastColor(controlItemBgActive)
              .setA(0.2)
              .toHexString(),
          },
