...
import type {
  MultipleSelectorToken,
  SelectorToken,
} from '../../select/style/token';
...
export interface ComponentToken
  extends
    ArrowToken,
    Exclude<SharedComponentToken, 'addonBg'>,
    PanelComponentToken {
  /**
   * @desc 预设区域最大宽度
...
export type SharedPickerToken = PanelComponentToken &
  PickerPanelToken &
...
export function initPickerPanelToken(
  ...
  return {
    ...
    pickerQuarterPanelContentHeight: token
      .calc(controlHeightLG)
      .mul(1.4)
      .equal(),
    pickerCellPaddingVertical: token
      .calc(paddingXXS)
...
export function initPanelComponentToken(
  ...
  const filledToken = {
    ...
    cellHoverWithRangeBg: new FastColor(token.colorPrimary)
      .lighten(35)
      .toHexString(),
    cellRangeBorderColor: new FastColor(token.colorPrimary)
      .lighten(20)
      .toHexString(),
...
export const prepareComponentToken: GetDefaultToken<'DatePicker'> = (
  token,
) => ({
