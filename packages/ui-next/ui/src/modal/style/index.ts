...
> = (token) => {
  ...
  return [
    {
      [`${componentCls}-root`]: {
        [`${componentCls}${asCls}-zoom-enter, ${componentCls}${asCls}-zoom-appear`]:
          {
    ...
    // ======================== Modal ========================
    {
      [componentCls]: {
        ...
        [`${componentCls}-footer`]: {
          ...
          [`> ${token.asCls}-btn + ${token.asCls}-btn`]: {
...
const genResponsiveWidthStyle: GenerateStyle<ModalToken, CSSObject> = (
  token,
) => {
  const { componentCls } = token;
  ...
  const responsiveStyles = Object.entries(gridMediaSizesMap).map(
    ([key, value]) => ({
      [`@media (min-width: ${unit(value!)})`]: {
