...
import type { CssUtil } from '../../_util/motion';
import type {
  FullToken,
  GenerateStyle,
  GetDefaultToken,
...
export interface ComponentToken {
   ...
   */
  groupTitleLineHeight: number | string;
  /**
   * @desc 横向菜单项圆角
        ...
        [`${componentCls}-item-icon + ${componentCls}-title-content-with-extra`]:
          {
            width: `calc(100% - ${unit(
              token
                .calc(token.iconSize)
                .add(token.iconMarginInlineEnd ?? 0)
                .equal(),
            )})`,
          },
...
export const prepareComponentToken: GetDefaultToken<'Menu'> = (token) => {
  ...
  const colorTextDark = new FastColor(colorTextLightSolid)
    .setA(0.65)
    .toRgbString();
  ...
  return {
    ...
    // Disabled
    darkItemDisabledColor: new FastColor(colorTextLightSolid)
      .setA(0.25)
      .toRgbString(),
...
export default (
  ...
  rootCls: ComputedRef<string> = prefixCls,
  injectStyle = true,
  ...
  const useStyle = genStyleHooks(
    ...
    (token) => {
      ...
      // Menu Token
      const menuToken = mergeToken<CssUtil & MenuToken>(token, {
        menuArrowSize,
