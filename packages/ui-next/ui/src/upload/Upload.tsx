...
import type { UploadProps as VcUploadProps } from '@arvin-studio/headless';
...
import { filterEmpty, Upload as VcUpload } from '@arvin-studio/headless';
...
export interface InternalUploadProps /* @vue-ignore */
  extends UploadEmitsProps, UploadProps {}
...
>(
  (props = defaults, { slots, attrs, expose, emit }) => {
    ...
    const customRequest = computed(
      () => props?.customRequest ?? contextCustomRequest.value,
    );
    ...
    const internalFileList = shallowRef(
    ...
    const mergedFileList = computed(
      () => props?.fileList ?? internalFileList.value ?? [],
    );
    ...
    const mergedBeforeUpload = async (file: VcFile, fileListArgs: VcFile[]) => {
      ...
      let parsedFile: Blob | File | string = file;
      if (beforeUpload) {
    ...
    const onBatchStart: VcUploadProps['onBatchStart'] = (
      batchFileInfoList: any,
    ) => {
      // Skip file which marked as `LIST_IGNORE`, these file will not add to file list
      const filteredFileInfoList = batchFileInfoList.filter(
        (info: any) => !(info.file as any)[LIST_IGNORE],
      );

      // Nothing to do since no file need upload
      if (filteredFileInfoList.length === 0) {
        return;
      }

      const objectFileList = filteredFileInfoList.map((info: any) =>
      ...
      objectFileList.forEach((fileObj: any) => {
        // Replace file if exist
        newFileList = updateFileList(fileObj, newFileList);
      });

      objectFileList.forEach((fileObj: any, index: any) => {
      ...
      Promise.resolve(
        ...
        if (removedFileList) {
          ...
          mergedFileList.value?.forEach((item) => {
            const matchKey = currentFile.uid === undefined ? 'name' : 'uid';
            if (
    ...
    const renderUploadList = (button?: any, buttonVisible?: boolean) => {
      ...
      const {
        ...
        typeof showUploadList.value === 'boolean' ? {} : showUploadList.value
      ) as ShowUploadListInterface;

      const realShowRemoveIcon =
        showRemoveIcon === undefined ? !mergedDisabled.value : showRemoveIcon;

      const iconRender = slots.iconRender
        ? (file: UploadFile, currentListType?: any) =>
            slots.iconRender?.({ file, listType: currentListType })
        : props.iconRender;
    ...
    return () => {
      ...
      const mergedRootCls = clsx(
        ...
        hashId.value,
        {
          ...
          [`${prefixCls.value}-picture-circle-wrapper`]:
          ...
          >
            ...
            >
              <VcUpload
                ...
                ref={upload}
              >
                <div class={`${prefixCls.value}-drag-container`}>
                  {children}
                </div>
              </VcUpload>
            </div>
