    ...
    parse: (locale, text, formats) => {
      ...
      for (let format of formats) {
        ...
        if (format.includes('wo') || format.includes('Wo')) {
          format = format.replaceAll('wo', 'w').replaceAll('Wo', 'W');
          ...
          if (matchFormat && matchText) {
            ...
            fallbackFormatList.push(format.replaceAll('o', ''));
          }
        }
      ...
      for (const element of fallbackFormatList) {
        const date = moment(text, element, locale, false);
