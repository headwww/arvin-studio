/**
 *
 * 通过 UA（User-Agent）字符串判断当前环境是否为移动设备，
 * 供组件做移动端适配（如弹层手势、触屏事件等）使用。
 * 关键设计：
 * 1. SSR 安全：navigator / window 不存在时直接返回 false；
 * 2. 两段式正则：第一段匹配移动端系统/浏览器标识，第二段匹配设备型号
 *    特征码（只取 UA 前 4 位做前缀匹配，命中率高、开销小）；
 * 3. 结果不缓存：每次调用实时计算。
 */
const getIsMobile = () => {
  // SSR / 非浏览器环境：不存在 navigator / window 视为非移动端
  if (typeof navigator === 'undefined' || typeof window === 'undefined')
    return false;

  // 取 UA 原文（兼容 vendor / opera 等旧字段兜底）
  const agent =
    navigator.userAgent || navigator.vendor || (window as any).opera;
  return (
    // 第一段：匹配移动端系统/浏览器标识（Android、iPhone、iPad、Opera Mini 等）
    /(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino|android|ipad|playbook|silk/i.test(
      agent,
    ) ||
    // 第二段：匹配设备型号特征码（取 UA 前 4 位前缀，如 "Nok"、"Sams" 等）
