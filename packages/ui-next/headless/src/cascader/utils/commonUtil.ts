...
export const VALUE_SPLIT = '__HEADLESS_CASCADER_SPLIT__';
...
export function scrollIntoParentView(element: HTMLElement) {
  ...
  if (elementToParent < parent.scrollTop) {
...
export function getFullPathKeys(
  ...
  return options.map((item) =>
    item[SEARCH_MARK]?.map(
      (opt: Record<string, any>) => opt[fieldNames.value as string],
    ),
  );
}
