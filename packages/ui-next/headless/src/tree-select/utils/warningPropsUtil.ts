...
export default function warningProps(
  props: TreeSelectProps & { searchPlaceholder?: string },
) {
  const {
    searchPlaceholder,
    treeCheckStrictly,
  ...
  if (labelInValue || treeCheckStrictly) {
    warning(
      toArray(value as any).every(
        (val: any) => val && typeof val === 'object' && 'value' in val,
      ),
