...
export type DraftValueType =
  | (DisplayValueType | LabelInValueType | RawValueType)[]
  | DisplayValueType
  | LabelInValueType
  | RawValueType;
...
const Select = defineComponent<SelectProps>({
  ...
  setup(props = defaults, { attrs, expose, slots }) {
    ...
    const mergedFilterOption = computed(() => {
      if (
        searchConfig.value.filterOption === undefined &&
        props.mode === 'combobox'
      ) {
        return false;
      }
      ...
      return valueList.map((val) => {
        ...
        if (option) {
          ...
          if (rawLabel === undefined) {
            rawLabel =
              option?.[props.optionLabelProp || mergedFieldNames.value.label];
          }
    ...
    const rawLabeledValues = computed(() => {
      const newInternalValue =
        multiple.value && internalValue.value === null
          ? []
          : internalValue.value;
    ...
    const rawValues = computed(
      () => new Set(mergedValues.value.map((val) => val.value)),
    );
    ...
    const filledSearchOptions = computed(() => {
      ...
      if (
        filteredOptions.value.some(
          (item) =>
            item[mergedFieldNames.value.value] === mergedSearchValue.value,
        )
      ) {
        return filteredOptions.value;
      }
      ...
      return [
        createTagOption(mergedSearchValue.value),
        ...filteredOptions.value,
      ];
    });

    const sorter = (inputOptions: DefaultOptionType[]): DefaultOptionType[] => {
      const sortedOptions = [...inputOptions].toSorted((a, b) =>
    ...
    const triggerChange = (values: DraftValueType) => {
      ...
      if (
        ...
        (labeledValues.length !== prevValues.length ||
          labeledValues.some(
            (newVal, index) => prevValues[index]?.value !== newVal?.value,
          ))
      ) {
    ...
    // ======================= Accessibility ========================
    const activeValue = shallowRef<null | string>(null);
    ...
    const onInternalSelect = useRefFunc<OnInternalSelect>((val, info) => {
      let cloneValues: (DisplayValueType | RawValueType)[];
      ...
      if (info.source === 'submit') {
        ...
        if (formatted) {
          ...
          const newRawValues = Array.from(
            new Set<RawValueType>([...rawValues.value, formatted]),
          );
    ...
    const onInternalSearchSplit: BaseSelectProps['onSearchSplit'] = (words) => {
      ...
      const newRawValues = Array.from(
        new Set<RawValueType>([...rawValues.value, ...patchValues]),
      );
      triggerChange(newRawValues);
    ...
    const selectContext = computed(() => {
      const realVirtual =
        props.virtual !== false && props.popupMatchSelectWidth !== false;
      return {
        ...parsedOptions.value,
    ...
    return () => {
      ...
      if (!props.options || props.options.length === 0) {
        ...
        // 生成一个简单的 key 来比较是否变化
        const newKey = newChildrenOptions
          .map((o: any) => `${o.value}`)
          .join(',');
