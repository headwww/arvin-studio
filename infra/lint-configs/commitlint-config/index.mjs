...
function getPackagesSync() {
  ...
  const patterns = match[1]
    ...
    .map((line) => line.trim().slice(2).replaceAll(/['"]/g, ''));

  const pkgs = [];

  for (const pattern of patterns) {
    // 拆分路径，例如 'infra/lint-configs/*' → ['infra', 'lint-configs', '*']
    ...
    for (let i = 0; i < segments.length; i++) {
      ...
      if (seg === '*') {
        ...
        for (const sub of subDirs) {
          let pkgDir = resolve(dir, sub.name);
          // 如果 * 后面还有路径，拼上
          for (const r of remaining) {
            pkgDir = resolve(pkgDir, r);
          }
